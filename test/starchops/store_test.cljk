...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:product-type :starch/corn-native :moisture-percent 12.0}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:product-type :starch/corn-native}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:product-type :starch/corn-native :moisture-percent 12.0}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:product-type (get-in result [:batches "batch-001"])) :starch/corn-native))
      (is (= (:moisture-percent (get-in result [:batches "batch-001"])) 12.0)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:product-type :starch/corn-native}}}
