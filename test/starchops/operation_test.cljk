...
(deftest run-operation-op-not-allowed-test
  (testing "an out-of-allowlist op (e.g. direct extraction-line control) is a hard, permanent block"
    (let [store {:batches {"batch-005" clean-batch}}
          request {:op :control-extraction-line :subject "batch-005"}
          proposal {:cites [{:spec "Centrifuge-Manual"}]
                    :value {:jurisdiction :jp/prefectural}
                    :effect :propose
                    :confidence 0.99}
          context {:actor-id "op-1" :hold-fact-fn governor/hold-fact}
